...
while menu_choice != 5:
    ...
    if menu_choice == 1: #Add student
        ...
        if name in students:
            ...
            which = 1234
            while which != -1: #If user enters 0 to break, it becomes -1 due to 'which=which-1'in the code block below
                ...
                if 0 <= which < len(grades): # Check if index is within range
                    grade = input("Grade: ")
                    grades[which] = grade
                elif which != -1:  #Triggered for any input that isn't 0 and hasn't already been dealt with 
